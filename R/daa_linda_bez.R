...
linda_obj <- linda(
  asv_tab, 
  meta, 
  formula = '~condition * age_s + (1|skippy_id)', 
  alpha = 0.4,
  prev.cut = 0.1, 
  lib.cut = 1000, 
  winsor.quan = 0.97
...
asv_tab <- asv_tab[, rownames(meta)]
dim(na.omit(meta))
linda_obj <- linda(asv_tab, meta, formula = '~condition * age_s + (1|skippy_id)', alpha = 0.4,
...
meta <- colData(tse_year1) %>% as.data.frame() %>%
  select(skippy_id, age_s, all_of(fvars))
asv_tab <- asv_tab[, rownames(meta)]
linda_obj <- linda(asv_tab, meta, formula = '~condition * age_s', alpha = 0.4,
                   prev.cut = 0.1, lib.cut = 1000, winsor.quan = 0.97)
...
save(
  ...
  tables_linda_itt2, 
  linda_identified, 
  file = here::here("data/tables_linda_itt.Rds")
)

save(
  tables_linda_itt_infancy, 
  file = here::here("data/tables_linda_itt_infancy.Rds")
)

save(
  tables_linda_itt_year1, 
  file = here::here("data/tables_linda_itt_year1.Rds")
) 
...
models_imp <- map2(implist, 1:length(implist), function(dimp, imp) {
  ...
  colData(tse_map) <- colData(tse_map) %>%
    as.data.frame() %>%
    select(-siblings, -bfexcl) %>%
    left_join(select(dimp, age, sample_id, siblings, bfexcl), by = "sample_id") %>%
    column_to_rownames("sample_id") %>%
    mutate(across(all_of(fvars), function(x) as.factor(x))) %>%
    DataFrame()
  colData(tse_map)$age <- colData(tse_map)$age + as.numeric(colData(tse_map)$week) * 7
  ...
  asv_tab <- asv_tab[, rownames(meta)]
  linda_obj <- linda(asv_tab, meta, formula = '~condition * age_s + siblings + bfexcl', alpha = 0.4,
                     prev.cut = 0.1, lib.cut = 1000, winsor.quan = 0.97)
  res <- filter(linda_obj$output$condition1, reject) %>%
    rownames_to_column("taxid")
  if (dim(res)[1] >= 1) {
...
save(tables_linda_pp, file = here::here("data/tables_linda_pp.Rds"))

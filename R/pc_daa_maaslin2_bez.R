...
colData(tse) <- colData(tse) %>%
  as.data.frame() %>%
  select(-all_of(fvars), -age, -age_s) %>%
  left_join(select(d, age, sample_id, constipation, diarrhea, siblings, condition), by = "sample_id") %>%
...
fit_data <- Maaslin2(
  ...
  transform = "AST",
  fixed_effects = c("condition", "siblings", "age_s"),
...
if (!file.exists(here::here("data/maaslin2_itt_mi_pc_bez.Rds"))) {
  ...
  save(models_imp, file = here::here("data/maaslin2_itt_mi_pc_bez.Rds"))
 } else {
  load(here::here("data/maaslin2_itt_mi_pc_bez.Rds"))
 }
...
modules
colnames(modules) <- c("gbm", "description")
...
maaslin2_identified <- map2_dfr(maaslin2_tables_itt, c("all", "early", "late"), function(table, term) {
  ...
  rename(gbm = feature) %>%
  left_join(modules, by = "gbm") %>%
  select(-gbm) %>%
  select(gbm = description, everything())

maaslin2_identified
save(maaslin2_tables_itt, maaslin2_identified,  file = here::here("data/maaslin2_tables_itt_pc.Rds"))

map2_dfr(maaslin2_tables_itt, c("all", "early", "late"), function(table, term) {
  filter(table, qval <= 0.2) %>%
  mutate(term = term) %>%
  arrange(coef, qval)
  }) %>%
...
tse <- transformSamples(tse, abund_values = "relabundance", method = "log10", pseudocount = 1)
gbms_identified <- as.data.frame(t(assay(tse, "log10"))) %>%
...
# colnames(gbms_identified) <- c(
#   "sample_id", 
#   "Nitric oxide degradation I (NO dioxygenase)",
#   "Butyrate synthesis I",
#   "Acetate synthesis III"
# )
gbms_meta <- as.data.frame(colData(tse)) %>%
  rownames_to_column("sample_id") %>%
  mutate(condition_label = ifelse(condition == 0, "CAU", ifelse(
    condition == 1, "SSC", NA)),
    week_label = glue::glue("Week {week}")
    ) %>%
  select(sample_id, condition_label, week_label)

gbms_data <- full_join(gbms_identified, gbms_meta, by = "sample_id") 
plots <- map2(list(
  "MGB027",
  "MGB052", 
  "MGB045"),
  list(
  "Nitric oxide degradation I",
  "Butyrate synthesis I",
  "Acetate synthesis III"
  ),
  function(gbm_name, description) {
    #pivot_longer(-c(sample_id, condition_label, week_label), names_to = "GBM", values_to = "values") %>%
    ggplot(gbms_data, aes_string(x = "condition_label", y = gbm_name, fill = "condition_label")) +
...
save(plots, file = here::here("data/gbm_plots.Rds"))
...
colData(tse) <- colData(tse) %>%
  as.data.frame() %>%
  select(-siblings, -ges_age, -birthweight, -edlevel, -csection, -sex, -age) %>%
  left_join(select(d_pp, age, sample_id, constipation, diarrhea, siblings, pp, csection, sex, ges_age, edlevel, birthweight), by = "sample_id") %>%
  column_to_rownames("sample_id") %>%
    mutate(
...
if (!file.exists(here::here("data/maaslin2_pp_mi_pc_bez.Rds"))) {
  models_imp <- map2(implist_pp, 1:length(implist), function(dimp, imp) {
    ...
    colData(tse_map) <- colData(tse_map) %>%
      as.data.frame() %>%
      select(-ges_age, -birthweight, -edlevel, -all_of(fvars), -age, -age_s) %>%
      left_join(select(dimp, age, sample_id, constipation, diarrhea, siblings, condition, pp, csection, sex, ges_age_s, edlevel_s, birthweight_s), by = "sample_id") %>%
      column_to_rownames("sample_id") %>%
      mutate(across(all_of(fvars), function(x) as.factor(x))) %>%
      DataFrame()
    colData(tse_map)$age <- colData(tse_map)$age + as.numeric(colData(tse_map)$week) * 7
  ...
  save(models_imp, file = here::here("data/maaslin2_pp_mi_pc_bez.Rds"))
 } else {
  load(here::here("data/maaslin2_pp_mi_pc_bez.Rds"))
 }
...
maaslin2_tables_pp <- map(models_imp[[1]], function(x) {
    x %>% filter(metadata == "condition") %>%
      select(feature, coef, stderr, pval, qval) %>%
      arrange(qval, desc(abs(coef))) %>%
      mutate(
...
save(maaslin2_tables_pp, file = here::here("data/maaslin2_tables_pp_pc.Rds"))
